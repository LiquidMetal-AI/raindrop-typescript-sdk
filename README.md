...
async function main() {
  const searchResponse = await client.search.perform({ input: 'REPLACE_ME', request_id: 'REPLACE_ME' });
...
async function main() {
  const params: Raindrop.SearchPerformParams = { input: 'REPLACE_ME', request_id: 'REPLACE_ME' };
  const searchResponse: Raindrop.SearchResponse = await client.search.perform(params);
}
...
await client.object.upload('key', { bucket: 'bucket', body: fs.createReadStream('/path/to/file') });

// Or if you have the web `File` API you can pass a `File` instance:
await client.object.upload('key', { bucket: 'bucket', body: new File(['my bytes'], 'file') });

// You can also pass a `fetch` `Response`:
await client.object.upload('key', { bucket: 'bucket', body: await fetch('https://somesite/file') });

// Finally, if none of the above are convenient, you can use our `toFile` helper:
await client.object.upload('key', { bucket: 'bucket', body: await toFile(Buffer.from('my bytes'), 'file') });
...
async function main() {
  const searchResponse = await client.search
    .perform({ input: 'REPLACE_ME', request_id: 'REPLACE_ME' })
    .catch(async (err) => {
...
await client.search.perform({ input: 'REPLACE_ME', request_id: 'REPLACE_ME' }, {
  maxRetries: 5,
});
```
...
const response = await client.search.perform({ input: 'REPLACE_ME', request_id: 'REPLACE_ME' }).asResponse();
...
We are keen for your feedback; please open an [issue](https://www.github.com/LiquidMetal-AI/raindrop-typescript-sdk/issues) with questions, bugs, or suggestions.
